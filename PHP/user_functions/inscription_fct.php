...
if(isset($_POST['inscription'])){//l'utilisateur à cliqué sur "S'inscrire", on demande donc si les champs sont défini avec "isset"
  if(empty($_POST['surname'])){//le champ pseudo est vide, on arrête l'exécution du script et on affiche un message d'erreur
    ...
    if ($result_img !== false) {
      ...
      if(!mysqli_query($mysqli,"INSERT INTO detenteur SET nom_detenteur='".$_POST['surname']."', sire ='".$_POST['sire']."',prenom_detenteur ='".$_POST['name']."',mail_detenteur ='".$_POST['mail']."', password_detenteur='".hash('sha256', $_POST['password'])."'")){//on crypte le mot de passe avec la fonction propre à PHP: md5()
